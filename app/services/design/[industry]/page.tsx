...
import { ArrowLeft, CheckCircle, Phone, Mail, Palette, UtensilsCrossed, Stethoscope, ShoppingBag, Building, BookOpen } from 'lucide-react';
import { PageHeader } from '@/components/common/PageHeader';
import { CaseStudy } from '@/components/common/CaseStudy';
import { TestimonialGrid } from '@/components/common/TestimonialCard';
import { Button } from '@/components/ui/button';
import { getDesignIndustry, DESIGN_INDUSTRIES } from '@/lib/services-data';
...
export async function generateStaticParams() {
  return DESIGN_INDUSTRIES.map((industry) => ({
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const industry = getDesignIndustry(resolvedParams.industry);
  ...
  return generateSEOMetadata({
    title: `Website Design for ${industry.name} | Professional Design Services`,
    description: industry.fullDescription,
    keywords: ['website design', industry.name, 'web design services', 'UI/UX design'],
  });
}

export default async function DesignIndustryPage({ params }: PageProps) {
  const resolvedParams = await params;
  const industry = getDesignIndustry(resolvedParams.industry);
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: industry.name, url: `/services/design/${industry.slug}` },
  ]);

  const serviceSchema = generateServiceSchema(
    `Website Design for ${industry.name}`,
  ...
  return (
    <>
      ...
      <PageHeader
        title={`Website Design for ${industry.name}`}
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              <div className="bg-gray-900 rounded-2xl p-8 text-white">
                <h2 className="text-xl font-bold mb-6">Benefits for {industry.name}</h2>
                <div className="grid sm:grid-cols-2 gap-4">
              ...
              {industry.sampleScreenshots.length > 0 && (
                <div>
                  <h2 className="text-xl font-bold text-gray-900 mb-6">Sample Designs</h2>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {industry.sampleScreenshots.map((screenshot, index) => (
                      ...
                      >
                        <Image
                          src={screenshot}
                          alt={`${industry.name} design sample ${index + 1}`}
                          fill
                  ...
                  <p className="text-sm text-gray-500 mt-4 text-center">
                    These are sample designs. Your project will be custom-designed for your brand.
                  </p>
                </div>
              )}
              ...
              {industry.process && (
                <div>
                  <h2 className="text-xl font-bold text-gray-900 mb-6">Our Design Process</h2>
                  <div className="space-y-4">
                    {industry.process.map((step, index) => (
                      <div key={index} className="flex gap-4 items-start">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-edone-primary to-edone-secondary text-white flex items-center justify-center font-bold flex-shrink-0">
                          {step.step}
                        </div>
                        <div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden sticky top-24">
                <div className="bg-gradient-to-r from-edone-primary to-edone-secondary p-6 text-white">
                  <h3 className="text-xl font-bold mb-2">{industry.name} Design</h3>
                  <p className="text-white/80 text-sm">Professional website design</p>
                  {industry.priceRange && (
                ...
                <div className="p-6">
                  ...
                  <Button
                    ...
                    size="lg"
                  >
                    <Link href={`/contact?service=design&industry=${industry.slug}`}>
                      Get a Quote
                    </Link>
                  </Button>
                </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                ...
                <p className="text-gray-600 text-sm mb-6">
                  Let&apos;s discuss your {industry.name.toLowerCase()} website project.
                </p>
